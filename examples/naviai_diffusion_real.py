...
def receive_and_republish():
    """
    (Optional) 用于重新发布接收到的位姿和图像数据，便于调试或可视化。
    """
    rospy.loginfo("正在接收位姿数据...")
    pose_data = rospy.wait_for_message('/right_arm_tcp_pose', Pose, timeout=3.0)
    rospy.loginfo("接收到位姿数据，正在重新发布...")
    position_pub = rospy.Publisher('/new_right_arm_tcp_pose', Pose, queue_size=10)
    position_pub.publish(pose_data)
    rospy.loginfo("正在接收图像数据...")
    image_data = rospy.wait_for_message('/image_streaming', Image, timeout=3.0)
    rospy.loginfo("接收到图像数据，正在重新发布...")
    image_pub = rospy.Publisher('/new_image_streaming', Image, queue_size=10)
    image_pub.publish(image_data)
    rospy.loginfo("数据已重新发布。")
...
if __name__ == "__main__":
    # 加载预训练策略
    pretrained_policy_path = Path("/home/diffusion/src/lerobot-main/outputs/train/3000/030000/pretrained_model")
